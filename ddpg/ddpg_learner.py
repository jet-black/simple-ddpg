...
class ReplayBuffer:
    ...
    def add(self, item):
        ...
        if len(self.items) >= 5000:
            ...
            self.frozen_arr_idx = np.arange(0, self.frozen_items.shape[0])
            self.items = []
...
class Actor:
    ...
    def init_optimizer(self):
        ...
        self.optimizer = tf.train.RMSPropOptimizer(self.learning_rate).apply_gradients(zip(grads, self.actor_trainable))
...
class Critic:
    ...
    def init_optimizer(self):
        ...
        return loss, tf.train.RMSPropOptimizer(self.learning_rate).minimize(loss, var_list=self.critic_trainable)
...
def create_runner(save_path, state_dim, action_dim, discount=0.999, buffer_size=100000, batch_size=128):
    ...
    with g.as_default():
        ...
        actor = Actor(actor_x_input, state_dim, action_dim, 0.001)
        critic = Critic(y_labels, critic_x_input, state_dim, action_dim, 0.001)
